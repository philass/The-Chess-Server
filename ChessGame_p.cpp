...
class ChessGame 
{
  private:
    ...
    bool positionThreatened(int row, int col, std::vector<std::vector<int>> board){
      ...
      while (r<=7){
        ...
        if (board[r][c]==-5 && canMoveKing(r,col,row,col,board)){
          return true;
        }
        r++;
      }
      ...
      while (c <= 7){
        ...
        if (board[r][c]==-5 && canMoveKing(row,c,row,col,board)){
          return true;
        }
        c++;
      }
